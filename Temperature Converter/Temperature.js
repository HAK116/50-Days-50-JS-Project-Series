...
let fromAllOptions = document.querySelectorAll('.container  .from .from-options li');
...
let globalFromValue = 'Cel',globalToValue = 'Fahren';
...
let KelToFahren = (Number(c) - 273.15) * 9 / 5 + 32;        
 if(globalFromValue == 'Cel' && globalToValue == 'Kel'){
   formula.innerHTML = `${c}°C + 273.15 = <strong>${CelToKel.toFixed(2)}K</strong>`; 
   toInput.value = CelToKel.toFixed(2);
 }else if(globalFromValue == 'Kel' && globalToValue == 'Cel') {
   formula.innerHTML = `${c}K - 273.15 = <strong>${KelToCel.toFixed(2)}°C</strong>`;
   toInput.value = KelToCel.toFixed(2);
 }else if(globalFromValue == 'Fahren' && globalToValue == 'Cel') {
   formula.innerHTML = `(${c}F - 32) × 5/9 = <strong>${FahrenToCel.toFixed(2)}°C</strong>`;
   toInput.value = FahrenToCel.toFixed(2);
 }else if(globalFromValue == 'Cel' && globalToValue == 'Fahren') {
   formula.innerHTML = `(${c}°C × 9/5) + 32 = <strong>${CelToFahren.toFixed(2)}°F</strong>`;
   toInput.value = CelToFahren.toFixed(2);
 }else if(globalFromValue == 'Fahren' && globalToValue == 'Kel') {
   formula.innerHTML = `(${c}°F - 32) × 5/9 + 273.15 = <strong>${FahrenToKel.toFixed(2)}K</strong>`;
   toInput.value = FahrenToCel.toFixed(2);
 }else if(globalFromValue == 'Kel' && globalToValue == 'Fahren') {
   formula.innerHTML = `(${c}K - 273.15) × 9/5 + 32 = <strong>${KelToFahren.toFixed(2)}°F</strong>`;
